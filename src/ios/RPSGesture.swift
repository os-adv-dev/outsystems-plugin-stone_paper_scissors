...
private func isFingerExtended(
    mcp: Int, pip: Int, dip: Int,
    landmarks l: [NormalizedLandmark],
    straightThreshold: CGFloat = 160
) -> Bool {
    angleDeg(l[mcp], l[pip], l[dip]) >= straightThreshold
}

private func isThumbExtended(
    mcp: Int, ip: Int, tip: Int,
    landmarks l: [NormalizedLandmark],
    straightThreshold: CGFloat = 160
) -> Bool {
...
class RPSDetectionViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        if self.playersBgView == nil {
            ...
            NSLayoutConstraint.activate([
                ...
                bgView.bottomAnchor.constraint(
                    equalTo: self.player2Label.bottomAnchor, constant: 6),
            ])
        ...
        instructionsLabel.attributedText = NSAttributedString(
            string: instructionText, attributes: attributes)
        ...
        NSLayoutConstraint.activate([
            instructionsBgHighlight.leadingAnchor.constraint(
                equalTo: instructionsLabel.leadingAnchor, constant: -12),
            instructionsBgHighlight.trailingAnchor.constraint(
                equalTo: instructionsLabel.trailingAnchor, constant: 12),
            instructionsBgHighlight.topAnchor.constraint(
                equalTo: instructionsLabel.topAnchor, constant: -4),
            instructionsBgHighlight.bottomAnchor.constraint(
                equalTo: instructionsLabel.bottomAnchor, constant: 4),
        ])

        player1Label.attributedText = NSAttributedString(
            string: "Player 1: --", attributes: attributesPlayers)
        player2Label.attributedText = NSAttributedString(
            string: "Player 2: --", attributes: attributesPlayers)
        ...
        winnerLabel.attributedText = NSAttributedString(
            string: "",
            attributes: [
    ...
    private func denorm(_ p: CGPoint, to imageSize: CGSize, orientation: AVCaptureVideoOrientation)
        -> CGPoint
    {
    ...
    private func updateUI(with detections: [HandDetection]) {
        ...
        DispatchQueue.main.async {
            ...
            if sortedDetections.count >= 2 {
                let p1 = sortedDetections[0]
                let p2 = sortedDetections[1]
                self.player1Label.attributedText = NSAttributedString(
                    string: "Player 1: \(p1.gesture.rawValue)", attributes: attributesPlayers)
                self.player2Label.attributedText = NSAttributedString(
                    string: "Player 2: \(p2.gesture.rawValue)", attributes: attributesPlayers)
                if let win = self.determineWinner(p1.gesture, p2.gesture) {
                    //                    self.winnerLabel.morphingEffect = .fall
                    self.winnerLabel.attributedText = NSAttributedString(
                        string: win, attributes: attributesWinner)
                } else {
                    self.winnerLabel.attributedText = NSAttributedString(
                        string: "", attributes: attributesWinner)
                }
            } else if sortedDetections.count == 1 {
                self.player1Label.attributedText = NSAttributedString(
                    string: "Player 1: \(sortedDetections[0].gesture.rawValue)",
                    attributes: attributesPlayers)
                self.player2Label.attributedText = NSAttributedString(
                    string: "Player 2: Waiting...", attributes: attributesPlayers)
                self.winnerLabel.attributedText = NSAttributedString(
                    string: "", attributes: attributesWinner)
            } else {
                self.player1Label.attributedText = NSAttributedString(
    ...
    private func clearOverlays() {
        detectionOverlayView.subviews.forEach { $0.removeFromSuperview() }
    }
    ...
    private func drawDetections(_ detections: [HandDetection]) {
        ...
        for (idx, det) in sorted.enumerated() {
            let rectOnLayer =
                det.boundingBox.isEmpty ? layerRect(for: det.landmarks) : det.boundingBox
    ...
    private func convertImageRectToPreview(
        _ r: CGRect,
        imageSize: CGSize,
        ...
        let scale = max(
            previewBounds.width / imageSize.width,
    ...
    private func layerRect(for landmarks: [NormalizedLandmark]) -> CGRect {
        ...
        for lm in landmarks {
            ...
            let layerPt = pl.layerPointConverted(
...
extension RPSDetectionViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
        ...
        backgroundQueue.async { [weak self] in
            do {
                try self?.handLandmarker.detectAsync(image: mpImage, timestampInMilliseconds: ms)
...
extension RPSDetectionViewController: HandLandmarkerLiveStreamDelegate {
    func handLandmarker(
        ...
        for i in 0..<landmarks.count {
            ...
            print(
                "👉 Hand \(i): \(handLabel)  \(gesture.rawValue)  hand=\(pct)%  box=\(Int(screenRect.origin.x)),\(Int(screenRect.origin.y)),\(Int(screenRect.width)),\(Int(screenRect.height))"
            )
    ...
    static func instantiateFromStoryboard() -> RPSDetectionViewController {
        ...
        guard
            let vc = storyboard.instantiateViewController(
                withIdentifier: "RPSDetectionViewController") as? RPSDetectionViewController
        else {
